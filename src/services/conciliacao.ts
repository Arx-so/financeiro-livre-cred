...
export interface ReconciliationSummary {
  ...
  // Valores monetários para a view
...
export async function findMatchCandidates(
    ...
    for (const statement of pendingStatements) {
        for (const entry of entries) {
            ...
            const typeMatch = (statement.type === 'credito' && entry.type === 'receita')
        || (statement.type === 'debito' && entry.type === 'despesa');
            ...
            const descMatch = statement.description.toLowerCase().includes(entry.description.toLowerCase().substring(0, 10))
...
export async function getReconciliationSummary(bankAccountId: string): Promise<ReconciliationSummary> {
    ...
    const summary = (data || []).reduce(
        (acc, statement) => {
            const value = Number(statement.value) || 0;
            acc.total++;
            acc.total_statements += value;

            switch (statement.reconciliation_status) {
                case 'conciliado':
                    acc.reconciled++;
                    acc.reconciled_amount += value;
                    break;
                case 'pendente':
                    acc.pending++;
                    acc.pending_amount += value;
                    break;
                case 'divergente':
                    acc.divergent++;
                    break;
    ...
    summary.reconciliationRate = summary.total > 0
        ...
        : 0;
    summary.reconciliation_rate = summary.reconciliationRate;

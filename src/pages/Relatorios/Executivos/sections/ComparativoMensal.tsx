...
export function ComparativoMensal() {
    ...
    const { data, isLoading } = useQuery({
        queryKey: ['comparativo-periodo', unidadeAtual?.id, periodo, ano],
        queryFn: () => getComparativoPeriodoData(unidadeAtual!.id, periodo, ano),
    ...
    const handleExportPDF = () => {
        ...
        doc.setFontSize(18);
        doc.text(`Comparativo ${periodo.charAt(0).toUpperCase() + periodo.slice(1)}`, pageWidth / 2, 20, { align: 'center' });
        doc.setFontSize(10);
        ...
        doc.save(`comparativo-${periodo}-${ano}.pdf`);
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold text-foreground">Comparativo Mensal / Trimestral / Anual</h2>
                    <p className="text-muted-foreground">
                        Comparativo de resultados por período.
                    </p>
                </div>
                <div className="flex items-center gap-2">
                    <select
                        ...
                        value={periodo}
                        onChange={(e) => setPeriodo(e.target.value as 'mensal' | 'trimestral' | 'anual')}
                    >

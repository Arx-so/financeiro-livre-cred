...
export function useUpdateFinancialEntry() {
    ...
    return useMutation({
        mutationFn: ({ id, entry }: { id: string; entry: FinancialEntryUpdate }) => updateFinancialEntry(id, entry),
        onSuccess: (_, { id }) => {
            queryClient.invalidateQueries({ queryKey: financialKeys.all });
        },
    });
...
export function useMarkAsPaid() {
    ...
    return useMutation({
        mutationFn: ({ id, paymentDate }: { id: string; paymentDate?: string }) => markAsPaid(id, paymentDate),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: financialKeys.all });
        },
    });

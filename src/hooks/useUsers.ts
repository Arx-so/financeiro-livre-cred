...
export function useUpdateUser() {
    ...
    return useMutation({
        mutationFn: ({ userId, data }: { userId: string; data: ProfileUpdate }) => updateUser(userId, data),
        onSuccess: () => {
...
export function useSetUserBranchAccess() {
    ...
    return useMutation({
        mutationFn: ({ userId, branchIds }: { userId: string; branchIds: string[] }) => setUserBranchAccess(userId, branchIds),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['users'] });
        },
    });

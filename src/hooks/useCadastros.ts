...
export function useUpdateFavorecido() {
  ...
  return useMutation({
    mutationFn: ({ id, favorecido }: { id: string; favorecido: FavorecidoUpdate }) =>
      updateFavorecido(id, favorecido),
    onSuccess: (_, { id }) => {
      queryClient.invalidateQueries({ queryKey: cadastrosKeys.all });
    },
  });
...
export function useUploadFavorecidoPhoto() {
  ...
  return useMutation({
    mutationFn: ({ favorecidoId, file }: { favorecidoId: string; file: File }) =>
      uploadFavorecidoPhoto(favorecidoId, file),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: cadastrosKeys.all });
    },
  });
